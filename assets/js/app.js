...
(function () {
  ...
  var GOL = {
    ...
    randomState : function() {
      var x, y, i, alivecells;
      
      

      // Placw live cells
      aliveCells = (this.rows * this.columns) * 0.12;
      for (i = 0; i < aliveCells; i++) {
        x = this.helpers.random(0, this.columns - 1);
        y = this.helpers.random(0, this.rows - 1);
        this.listLife.addCell(x, y, this.listLife.actualState, false);
      }

      // Place undead cell
      x = this.helpers.random(0, this.columns - 1);
      y = this.helpers.random(0, this.rows - 1);
      this.listLife.addCell(x, y, this.listLife.actualState, true);
      ...
      nextGeneration : function() {
        ...
        for (i = 0; i < this.actualState.length; i++) {
          ...
          for (j = 1; j < this.actualState[i].length; j++) {
            x = this.actualState[i][j][0];
            y = this.actualState[i][0];
            isUndead = this.actualState[i][j][1];
